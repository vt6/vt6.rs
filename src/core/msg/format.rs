...
impl<'b> MessageFormatter<'b> {
    ...
    pub fn new(buffer: &'b mut [u8], type_name: &str, num_arguments: usize) -> Result<MessageFormatter<'b>, BufferTooSmallError> {
    ...
    ///Convenience wrapper around `new()` and `finalize()` that allows you to
    ///use the `?` operator on `add_argument()` even if your function's error
    ///type does not match ours. The result value is the same as for
    ///`finalize()`.
    ///
    ///```
    ///# use std;
    ...
    ///let size = MessageFormatter::format(&mut buf, "have", 2, |f| {
    ///    f.add_argument("core")?;
    ///    f.add_argument("1.0") //TODO update example after adding ModuleVersion
    ///}).unwrap();
    ///
    ///let msg_str = std::str::from_utf8(&buf[0..size]).unwrap();
    ///assert_eq!(msg_str, "{3|4:have,4:core,3:1.0,}");
    ///```
    pub fn format<T>(buffer: &'b mut [u8], type_name: &str, num_arguments: usize, callback: T) -> Result<usize, BufferTooSmallError>
        where T: FnOnce(&mut MessageFormatter) -> Result<(), BufferTooSmallError>
    {
        let mut f = Self::new(buffer, type_name, num_arguments)?;
        callback(&mut f)?;
        f.finalize()
    }
    ...
    ///`new()` or `format()`.
    pub fn add_argument<T: EncodeArgument + ?Sized>(&mut self, arg: &T) -> Result<(), BufferTooSmallError> {
    ...
    ///where `buffer` is the first argument passed to `new()` or `format()`.
    ///
    ///# Panics
    ///
    ///Panics if `add_argument()` has not been called sufficiently often (as
    ///often as announced in `new()` or `format()`) before this call.
    ...
    fn add_char(&mut self, c: u8) -> Result<(), BufferTooSmallError> {
    ...
    fn encode<T: EncodeArgument + ?Sized>(&mut self, arg: &T, size: usize) -> Result<(), BufferTooSmallError> {
        let new_cursor = self.cursor.wrapping_add(size);
        //check for integer overflow, buffer overflow
        if new_cursor < self.cursor || new_cursor > self.buffer.len() {
